"""Scan looming decay values and summarize recovery behavior.
...
SCAN_VALUES = [0.64, 0.72, 0.80, 0.88, 0.96]
...
def _run(task):
    ...
    try:
        ...
        setattr(s, "LOOMING_BOOST_DECAY", val)
...
def main():
    ...
    out_csv = OUT_DIR / "decay_sweep.csv"
    ...
    with open(out_csv, "w", newline="", encoding="utf-8-sig") as f:
        ...
        w.writerow(["LOOMING_BOOST_DECAY"] + KEYS)
    ...
    for ax, (k, cn) in zip(axes.flat, panels):
        ...
        ax.axvline(0.80, color="#999", ls="--", lw=0.8)

        ax.set_xlabel("LOOMING_BOOST_DECAY")

        ax.set_title(cn, fontsize=10)

    fig.suptitle(
        "LOOMING_BOOST_DECAY 扫描 0.64–0.96（恢复联动已启用）（虚线=默认0.80；负荷联动+自适应阈值已启用）",
        fontsize=11,
    )

    fig.tight_layout()

    png = OUT_DIR / "decay_sweep.png"
